...
const stats: Stat[] = [
  ...
  { value: '190+', label: 'Projects Delivered', gold: true },
  { value: '6', label: 'Cities Across India' },
  { value: '14', label: 'Design Awards', gold: true },
];
...
export const StudioSection: React.FC = () => {
  ...
  const rotateX = useSpring(useTransform(mouseY, [-0.5, 0.5], [14, -14]), { damping: 18, stiffness: 220 });
  const rotateY = useSpring(useTransform(mouseX, [-0.5, 0.5], [-14, 14]), { damping: 18, stiffness: 220 });

  const spotlightBg = useTransform(
    [spotlightX, spotlightY],
    ([x, y]) => `radial-gradient(circle 240px at ${x}px ${y}px, rgba(255,255,255,0.32), rgba(212,175,55,0.16), transparent 80%)`
  );
  ...
  return (
    <section
      id="studio"
      className="relative w-full min-h-screen bg-black text-[#E8DFD8] selection:bg-[#cbb59d] selection:text-black py-24 lg:py-32 px-6 sm:px-10 lg:px-14 overflow-hidden flex items-center"
    >
      {/* Ambient glows */}
      <motion.div
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.p
              ...
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              <span className="text-[#F3DBB3] font-medium">Lines &amp; Designs</span> is a
              Bengaluru interior design studio shaping homes, workplaces and hospitality
              spaces across India. For over a decade we've paired a restrained,
              material-first aesthetic with rigorous, on-time delivery — proving that
              calm, considered spaces and dependable execution can live under one roof.
            </motion.p>
            ...
            <motion.p
              ...
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              We take on a limited number of projects each year, so every space gets the
              attention it deserves — from the first moodboard to the last styled shelf.
            </motion.p>
                ...
                <div key={stat.label} className="flex flex-col">
                  <span
                    className={`text-3xl sm:text-4xl font-light tracking-tight ${
                      stat.gold ? 'text-[#D4AF37]' : 'text-[#F4EBE2]'
                    }`}
                    style={{ fontFamily: "'Bebas Neue', sans-serif" }}
                  >
                    {stat.value}
          ...
          {/* RIGHT 3D CARD */}
          <div className="lg:col-span-5 flex items-center justify-center relative perspective-[1400px] order-1 lg:order-2">
            <motion.div
              animate={{
            ...
            >
              ...
              {/* Image canvas */}
              <div className="relative overflow-hidden w-full max-w-[400px] aspect-[4/5] bg-black rounded-sm">
                <SmartImage
                  src={IMAGES.studio}
                  alt="Inside the Lines & Designs studio"
                  label="THE STUDIO"
                  className="absolute inset-0 w-full h-full"
                  imgClassName="object-center brightness-[0.94] contrast-[1.06] saturate-[1.02] group-hover:brightness-105 group-hover:contrast-[1.12] transition-all duration-700 ease-out"
                />
                ...
                {/* Caption + signature */}
                <div className="absolute bottom-4 left-4 right-4 z-20 flex items-end justify-between select-none">
                  <span
                    className="text-[9.5px] font-medium tracking-[0.28em] uppercase text-[#E0D3C5]"
                    style={{ fontFamily: "'Montserrat', sans-serif" }}
                  >
                    Bengaluru · Est. 2015
                  </span>
                  <span
                    className="text-3xl text-[#F2D8A7] drop-shadow-[0_0_12px_rgba(242,216,167,0.5)] transition-colors duration-300 group-hover:text-white leading-none"
